...
void draw_agent_window(ShellState* state) {
    ...
    const std::string review_count_label = state->agent_review_queue.empty()
        ? "clear"
        : std::to_string(state->agent_review_queue.size());
    if (widgets::section_header("Review", review_count_label.c_str())) {
        ...
        if (state->agent_review_queue.empty()) {
            ImGui::Dummy(ImVec2(0.0f, 8.0f));
            ImGui::TextColored(t::kFaint, "No pending file writes.");
        } else {
            std::optional<std::uint64_t> remove_review_id;
            for (const AgentReviewRequest& request : state->agent_review_queue) {
            ...
            if (remove_review_id.has_value()) {
                state->agent_review_queue.erase(
                    std::remove_if(
                        state->agent_review_queue.begin(),
                        state->agent_review_queue.end(),
    ...
    if (widgets::section_header("Activity", running ? "live" : "off")) {
        ...
        if (state->agent_activity_log.empty()) {
            ImGui::Dummy(ImVec2(0.0f, 8.0f));
            ImGui::TextColored(t::kFaint, "No agent operations yet.");
        } else {
            for (auto it = state->agent_activity_log.rbegin();
                 it != state->agent_activity_log.rend();
                 ++it) {
                ImGui::TextColored(
